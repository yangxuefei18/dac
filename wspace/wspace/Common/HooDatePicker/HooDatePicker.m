...
static NSString  * const kTodayButtonItemTitle = @"至今";
...
#define kHooDatePickerHighlightColor [UIColor colorWithRed:41.0/255.0 green:161.0/255.0 blue:247.0/255.0 alpha:1.0]
...
- (instancetype)initToThisDayWithSuperView:(UIView*)superView {
...
- (void)actionButtonToday:(UIButton *)sender {
    
    [self dismiss];
    
    if ([self.delegate respondsToSelector:@selector(datePicker:didSelectedToday:)]) {
        [self.delegate datePicker:self didSelectedToday:nil];
    }
}
...
- (UIView *)headerView {
    if (!_headerView) {
        ...
        //toThisDay
        UIButton *todayButton = [[UIButton alloc] initWithFrame:CGRectMake(self.frame.size.width/2-kHooDatePickerButtonHeaderWidth/2, 0.0, kHooDatePickerButtonHeaderWidth, kHooDatePickerHeaderHeight)];
        [todayButton setTitle:kTodayButtonItemTitle forState:UIControlStateNormal];
        [todayButton setTitleColor:self.highlightColor forState:UIControlStateNormal];
        [todayButton addTarget:self action:@selector(actionButtonToday:) forControlEvents:UIControlEventTouchUpInside];
        [_headerView addSubview:todayButton];
