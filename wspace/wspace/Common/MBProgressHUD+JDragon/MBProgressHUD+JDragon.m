...
+ (MBProgressHUD*)createMBProgressHUDviewWithMessage:(NSString*)message isWindiw:(BOOL)isWindow
{
    ...
    hud.bezelView.color = [Common colorWithHexString:TLCOLOR];
...
+ (void)showTipMessageInWindow:(NSString*)message yOffset:(float)y color:(UIColor *)c
{
    [self showTipMessage:message isWindow:true timer:1.5 yOffset:y color:c];
}
+ (void)showTipMessageInView:(NSString*)message yOffset:(float)y color:(UIColor *)c
{
    [self showTipMessage:message isWindow:false timer:1.5 yOffset:y color:c];
}
+ (void)showTipMessageInWindow:(NSString*)message timer:(int)aTimer yOffset:(float)y color:(UIColor *)c

{
    [self showTipMessage:message isWindow:true timer:aTimer yOffset:y color:c];
}
...
+ (void)showTipMessageInView:(NSString*)message timer:(int)aTimer yOffset:(float)y color:(UIColor *)c
{
    [self showTipMessage:message isWindow:false timer:aTimer yOffset:y color:c];
}
...
+ (void)showTipMessage:(NSString*)message isWindow:(BOOL)isWindow timer:(int)aTimer
{
    ...
    hud.bezelView.color = [Common colorWithHexString:TDCOLOR];
    hud.label.textColor = [UIColor whiteColor];
    hud.label.font = [UIFont systemFontOfSize:15 weight:20];
    [hud hideAnimated:YES afterDelay:aTimer];
}

+ (void)showTipMessage:(NSString*)message isWindow:(BOOL)isWindow timer:(int)aTimer yOffset:(float)y color:(UIColor *)c
{
    MBProgressHUD *hud = [self createMBProgressHUDviewWithMessage:message isWindiw:isWindow];
    hud.mode = MBProgressHUDModeText;
    hud.label.textColor = [UIColor whiteColor];
